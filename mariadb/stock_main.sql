...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `小資股(不到10萬)` AS select `stock_main`.`stock_name` AS `股名`,`stock_main`.`done_price` AS `股價`,max(`stock_main`.`check_time`) AS `查詢時間` from `stock_main` where `stock_main`.`done_price` <= 100 group by `stock_main`.`stock_name` order by `stock_main`.`done_price`;
